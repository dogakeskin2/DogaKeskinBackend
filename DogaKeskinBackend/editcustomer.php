<?php
   ...
   if (!empty($_POST)) {
       ...
       $params = [$email, $name, $city, $district, $address, $_SESSION["user"]["id"]];
       ...
       if (!empty($password)) {
           ...
           $stmt = $db->prepare("UPDATE customeruser SET email = ?, password = ?, name = ?, city = ?, district = ?, address = ? WHERE id = ?");
           $params = [$email, $hash, $name, $city, $district, $address, $_SESSION["user"]["id"]];
       } else {
           $stmt = $db->prepare("UPDATE customeruser SET email = ?, name = ?, city = ?, district = ?, address = ? WHERE id = ?");
       }
...
<body>
    <h1>Edit Customer User Information</h1>
